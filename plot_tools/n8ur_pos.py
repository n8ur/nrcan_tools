...
a = astropy.io.ascii.read(open('xyztest.dat').readlines())
...
def make_plots(a,name,loc='upper left'):
    ...
    f,ax = pylab.subplots()
    ax.hist(xd,5000,density=True, histtype='step', cumulative=True, label='Cumulative Normalized %s'%(name))
    ax.legend(loc=loc)
    ax.grid()
...
def find_conf_int(a,name,loc='upper left'):
    ...
    f,ax = pylab.subplots()
    hd = ax.hist(xd,5000,density=True,histtype='step',cumulative=True,label='Cumulative Normalized %s'%(name))
